...
class Evaluator:
    ...
    def evaluate(self):
        ...
        for ax, bc_type in zip(axs.flatten(), self.bc_types):
            ...
            ax.set_xlabel('x')
            ax.set_ylabel('u(x)')
            for j in range(min(20, len(self.truth[bc_type]) // self.discretization)):
                ax.plot(self.x.cpu(), self.truth[bc_type][self.discretization * j:self.discretization * (j + 1)], 'g',
                        label='Ground Truth')
                ax.plot(self.x.cpu(), self.prediction[bc_type][self.discretization * j:self.discretization * (j + 1)],
                        'r',
                        label='Prediction')
